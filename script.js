// Переменные для восстановления пароля
let resetEmail = null;
let resetCode = null;
...
// Показать модалку восстановления пароля
...
async function sendResetCode() {
    ...
    if (!email.includes('@') || !email.includes('.')) {
        showForgotError('Введите корректный email');
        return;
    }
    ...
    try {
        ...
        if (response.ok) {
            ...
            showForgotError(data.error || 'Ошибка отправки кода');
        }
    } catch (error) {
        showForgotError('Ошибка соединения с сервером');
    }
}
...
async function resetPassword() {
    ...
    try {
        ...
        if (response.ok) {
            showForgotSuccess('Пароль успешно изменен! Теперь вы можете войти.');
            setTimeout(() => {
                hideModal(forgotPasswordModal);
                showModal(authModal);
            }, 2000);
        } else {
            showForgotError(data.error || 'Ошибка смены пароля');
        }
    } catch (error) {
        showForgotError('Ошибка соединения с сервером');
    }
...
// Показать ошибку в модалке восстановления
...
// Показать успех в модалке восстановления
