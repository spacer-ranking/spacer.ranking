...
// Настройка почтового транспорта (Gmail)
const transporter = nodemailer.createTransport({
    service: 'gmail',
    auth: {
        user: 'cyber.rank.esports@gmail.com',
        pass: 'hnjw ymqy qbse fjrq' // замените на реальный пароль приложения
    }
});
...
// Инициализация таблиц с добавлением поля role в team_members и rating=0 по умолчанию
function initDatabase() {
    db.serialize(() => {
        ...
        // Таблица команд (rating по умолчанию 0)
        ...
        // Таблица участников команды с ролью
        ...
        // Проверяем, есть ли колонка role в team_members (для старых баз)
        db.all("PRAGMA table_info(team_members)", (err, rows) => {
            if (err) return;
            const hasRole = rows.some(col => col.name === 'role');
            if (!hasRole) {
                db.run("ALTER TABLE team_members ADD COLUMN role TEXT DEFAULT 'Игрок'");
                console.log('Добавлена колонка role в team_members');
            }
        });
...
// ========== Восстановление пароля с отправкой на почту ==========
...
// Получение команды по ID (с ролями участников)
app.get('/api/teams/:id', (req, res) => {
    ...
    `, [teamId], (err, team) => {
        ...
        db.all(`
            SELECT u.id, u.username, u.avatar, tm.role
...
// Создание команды (только лидер) - рейтинг = 0
    ...
    db.get("SELECT * FROM teams WHERE name = ?", [name], (err, team) => {
        ...
        db.run("INSERT INTO teams (name, leader_id, rating) VALUES (?, ?, 0)",
            ...
            function(err) {
                ...
                // Добавляем лидера в команду с ролью "Капитан"
                db.run("INSERT INTO team_members (team_id, user_id, role) VALUES (?, ?, 'Капитан')",
...
// Загрузка аватара команды (только лидер)
...
app.delete('/api/teams/:teamId', authenticateToken, isLeader, (req, res) => {
    ...
    db.get("SELECT * FROM teams WHERE id = ? AND leader_id = ?", [teamId, req.user.id], (err, team) => {
        ...
        // Удаляем связанные записи (матчи, участники)
...
// Добавление игрока в команду (только лидер) с выбором роли
app.post('/api/teams/:teamId/members', authenticateToken, isLeader, (req, res) => {
    ...
    db.get("SELECT * FROM teams WHERE id = ? AND leader_id = ?", [teamId, req.user.id], (err, team) => {
        ...
        db.get("SELECT * FROM users WHERE username = ?", [username], (err, user) => {
            ...
            db.run("INSERT OR IGNORE INTO team_members (team_id, user_id, role) VALUES (?, ?, ?)",
                [teamId, user.id, role || 'Игрок'], function(err) {
...
// Обновление роли игрока в команде (только лидер)
app.put('/api/teams/:teamId/members/:userId', authenticateToken, isLeader, (req, res) => {
    const teamId = req.params.teamId;
    ...
    db.get("SELECT * FROM teams WHERE id = ? AND leader_id = ?", [teamId, req.user.id], (err, team) => {
        ...
        db.run("UPDATE team_members SET role = ? WHERE team_id = ? AND user_id = ?",
            [role, teamId, userId], function(err) {
...
// Удаление игрока из команды (только лидер)
app.delete('/api/teams/:teamId/members/:userId', authenticateToken, isLeader, (req, res) => {
    const teamId = req.params.teamId;
    const userId = req.params.userId;
    ...
    db.get("SELECT * FROM teams WHERE id = ? AND leader_id = ?", [teamId, req.user.id], (err, team) => {
        ...
        // Нельзя удалить самого лидера
        if (userId == req.user.id) {
            return res.status(400).json({ error: 'Нельзя удалить лидера команды' });
        }

        db.run("DELETE FROM team_members WHERE team_id = ? AND user_id = ?", [teamId, userId], function(err) {
...
// Удаление матча (только лидер, который создал матч)
app.delete('/api/matches/:matchId', authenticateToken, isLeader, (req, res) => {
    ...
    db.get("SELECT * FROM matches WHERE id = ? AND created_by = ?", [matchId, req.user.id], (err, match) => {
        ...
        // Перед удалением нужно откатить рейтинг? По желанию. Пока просто удалим.
